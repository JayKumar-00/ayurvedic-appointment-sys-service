...
export class PatientQueueController {
  ...
  findAll(@Req() req: { user: JwtPayload }) {
    return this.patientQueueService.findAll(req.user);
  }
  ...
  updateStatus(
    ...
    return this.patientQueueService.updateStatus(id, status, req.user);
  }
  ...
  remove(@Req() req: { user: JwtPayload }, @Param('id') id: string) {
    return this.patientQueueService.remove(id, req.user);
  }
}
